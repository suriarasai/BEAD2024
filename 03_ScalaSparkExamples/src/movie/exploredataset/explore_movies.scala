...
object explore_movies {
  ...
  def main(args: Array[String]) {
    ...
    val raw_title = MovieUtil.getMovieDataDF().select("name")
    raw_title.show()

    raw_title.createOrReplaceTempView("titles")
    MovieUtil.spark.udf.register("processRegex", processRegex _)
    val processed_titles = MovieUtil.spark.sql("select processRegex(name) from titles")
