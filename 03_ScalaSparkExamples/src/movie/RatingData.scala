...
object RatingData {
  def main(args: Array[String]) {
    ...
    val num_movies = MovieUtil.getMovieDataDF().count()
    val first = rating_df.first()
    println("first:" + first)
    println("num_ratings:" + num_ratings)

    val max = MovieUtil.spark.sql("select max(rating)  from df")
    max.show()

    val min = MovieUtil.spark.sql("select min(rating)  from df")
    min.show()

    val avg = MovieUtil.spark.sql("select avg(rating)  from df")
    avg.show()
    ...
    val movie_fields_df = MovieUtil.getMovieDataDF()
    val user_data_df = MovieUtil.getUserFieldDataFrame()
    ...
    MovieUtil.spark.udf.register("getCurrentHour", getCurrentHour _)

    val timestamps_df =  MovieUtil.spark.sql("select getCurrentHour(timestamp) as hour from df")
    timestamps_df.show()

    MovieUtil.spark.udf.register("assignTod", assignTod _)
    timestamps_df.createOrReplaceTempView("timestamps")
    val tod = MovieUtil.spark.sql("select assignTod(hour) as tod from timestamps")
    tod.show()
