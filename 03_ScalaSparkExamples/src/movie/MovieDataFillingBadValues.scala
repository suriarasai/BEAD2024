...
object MovieDataFillingBadValues {
  ...
  def main(args: Array[String]) {
    val movie_data_df = MovieUtil.getMovieDataDF()
    movie_data_df.createOrReplaceTempView("movie_data")
    movie_data_df.printSchema()

    MovieUtil.spark.udf.register("convertYear", MovieUtil.convertYear _)
    movie_data_df.show(false)

    val movie_years = MovieUtil.spark.sql("select convertYear(date) as year from movie_data")

    movie_years.createOrReplaceTempView("movie_years")
    MovieUtil.spark.udf.register("replaceEmptyStr", replaceEmptyStr _)

    val years_replaced =  MovieUtil.spark.sql("select replaceEmptyStr(year) as r_year from movie_years")
    ...
    val years_replaced_rdd = MovieUtil.sc.parallelize(years_replaced_int)
